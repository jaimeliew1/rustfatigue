...
use crate::{eq_load, eq_load_max_half_cycle_closed, get_peaktrough, get_halfcycles};
...
#[pyfunction]
pub fn eq_load_max_half_cycle_closed_python(signal: PyReadonlyArrayDyn<'_, f64>, m: f64, neq: u64, half: bool) -> f64 {
    eq_load_max_half_cycle_closed(&signal.to_vec().unwrap(), m, neq, half)
}
...
fn rustfatigue(_py: Python, m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(eq_load_max_half_cycle_closed_python, m)?)?;
